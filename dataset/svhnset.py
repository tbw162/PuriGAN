...
def create_loader(opt,kwargs):
    ...
    dataset1 = datasets.SVHN(root ='data-svhn', split='train', download=True,transform=data_transform)

    data1 = dataset1.data
    target1 = dataset1.labels
    ...
    num1 = data1[target1==anomaly_list[random_list[0]]].shape[0]
    dataset1.data = np.concatenate((data1_p[randIdx_normal[:num1]],data1_n[randIdx[:int(opt.gamma_p*num1//(1-opt.gamma_p))]]),axis=0)

    dataset1.labels = np.concatenate((target1_p[randIdx_normal[:num1]],target1_n[randIdx[:int(opt.gamma_p*num1//(1-opt.gamma_p))]]),axis=0)
 
    
    num2 = data1_n.shape[0]
    dataset2 = datasets.SVHN(root ='data-svhn', split='train', download=True,transform=data_transform)

    data2 = data1_n[randIdx[num2-num1//int((1/opt.gamma_c)):num2]]
    target2 = target1_n[randIdx[num2-num1//int((1/opt.gamma_c)):num2]]
    ...
    dataset4 = datasets.SVHN(root ='data-svhn', split='test', download=True,transform=data_transform)
    data4 = dataset4.data
    target4 = dataset4.labels
    if(opt.k1==1):
    ...
    dataset4.labels = target4_p[randIdx_test[:5000]]
