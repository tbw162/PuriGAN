...
def testing(model_inv,mu1,sigma1,generator,batches_done,opt,device):
    generator.eval()
    ...
    fake_images_all = []
    for i in range(0,10):
        z_out_fake = Variable(Tensor(np.random.normal(0, 1, (100, opt.latent_dim,1,1))))
        ...
        fake_images_all.append(gen)
        
    
    fake_images_all = torch.cat(fake_images_all)
