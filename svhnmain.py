...
parser.add_argument("--dataset", type=str, default='SVHN', help="choice of dataset(F-MNIST,MNIST,CELEBA,SVHN)")
...
from dataset.svhnset import create_loader
...
for epoch in range(opt.n_epochs):
    ...
    for (batch_pos,batch_neg) in zip(train_pos,cycle(train_neg)):
        ...
        if(epoch%100<epoch_pro):
            g_loss = adversarial_loss(discriminator(img_pos), c*valid) +adversarial_loss(discriminator(gen_imgs), c*valid) 
        else: 
        ...
        if(epoch%100>(epoch_pro-5) and batches_done%150==0):
